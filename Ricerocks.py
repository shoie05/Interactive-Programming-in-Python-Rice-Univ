...
missile_info = ImageInfo([8,8], [16, 16], 5, 50)
...
class Sprite:
    ...
    def draw(self, canvas):
        global time
        if self.animated :
            exp_index = (time % self.lifespan) // 1
            exp_center = [ self.image_center[0]+exp_index*self.image_size[0], self.image_center[1]]
            canvas.draw_image(self.image, exp_center, self.image_size,self.pos, self.image_size)
            time += 1
...
def group_collide(group, other_object):
    ...
    tmp_grp = list(group)
    for element in tmp_grp :
        if element.collide(other_object):
            group.remove(element)
            a_exp = Sprite(element.get_position(), [-1,1], 0, 0, explosion_image, explosion_info, explosion_sound)
            explosion_group.append(a_exp)
            return True    
